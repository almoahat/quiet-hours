# quiet-hours
**An agent that watches your logs so you don't have to **
...
I spent five years on call for distributed services and built the observability layer across 20+ Kubernetes services. This is the tool I wanted at 3am.
...
1. **Live connectors.** Adapters for real sources. The parser is the only source-specific code.
2. **GPU cluster telemetry.** The same pipeline against a failed distributed training run: which node died, and why. Different corpus, identical architecture.
3. **Concurrent incidents.** Multiple timelines at once, ranked by severity.
4. **Evaluation harness.** A corpus of scripted incidents with known causes, to measure diagnosis accuracy rather than eyeballing it.
